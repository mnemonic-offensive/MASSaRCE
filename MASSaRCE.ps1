...
function Connect-MassWinRM {
    ...
    foreach ($c in $ADComputers) {
        Write-Verbose "Trying to run command on $($c)"
        if ($Username){
            Invoke-Command -ComputerName $c -Credential $cred -ScriptBlock $Cmd -ErrorAction SilentlyContinue
        } else {
            Invoke-Command -ComputerName $c -ScriptBlock $Cmd -ErrorAction SilentlyContinue
        }
    }
}

function Connect-MassRDP {
    [CmdletBinding()]
    param(
...
# ---------------------------------------------------------------------------------------
# The Connect-Mstsc function by Jaap Brasser below is copied from 
# https://www.powershellgallery.com/packages/Connect-Mstsc/1.2.5 (last updated in 2016)
# It is imported here so the script can work in environments without Internet connection
# ---------------------------------------------------------------------------------------
